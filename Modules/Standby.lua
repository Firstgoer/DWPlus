	...
	if string.find(text, "!standby") == 1 and core.IsOfficer then
		...
		if cmd and cmd:gsub("%s+", "") ~= "nil" and cmd:gsub("%s+", "") ~= "" then
			...
			if search and not verify then
				...
				response = "DWPlus: "..cmd.." "..L["STANDBYWHISPERRESP1"]
			elseif search and verify then
				response = "DWPlus: "..cmd.." "..L["STANDBYWHISPERRESP2"]
			else
				response = "DWPlus: "..cmd.." "..L["STANDBYWHISPERRESP3"];
			end
		else
			...
			if search and not verify then
				table.insert(MonDKP_Standby, MonDKP_DKPTable[search[1][1]])
				response = "DWPlus: "..L["STANDBYWHISPERRESP4"]
			elseif search and verify then
				response = "DWPlus: "..L["STANDBYWHISPERRESP5"]
			else
				response = "DWPlus: "..L["STANDBYWHISPERRESP6"];
			end
		end
