if GetLocale() == "zhCN" then
  ...
  core.L = {
    ...
    MINUTES = "分钟",
    MISCSETTINGS = "杂项设置",
    DWPSCALESIZE = "整体式DKP秤尺寸",
    DWPSCALESIZETTDESC = "Monolith DKP窗口的比例。单击\"保存设置\"以将大小更改为设置值。",
    DWPSCALESIZETTWARN = "如果使用另一个修改UI比例的插件（例如TukUI，ElvUI等），则保存后可能需要/ reload。",
    MORE = "更多",
    ...
    强烈建议您仅在希望限制哪些警官需要权限的情况下才使用此白名单设置。如果您希望所有人员都具有权限，请完全忽略此设置功能。请谨慎使用。 （如果应用这些设置会导致问题，请使用文本编辑器打开\WTF\Accounts\ACCOUNT_NAME\SavedVariables\DWPlus.lua文件，然后删除底部附近的DWPlus_Whitelist表。）]=],
    WIPETABLES = "擦桌子",
