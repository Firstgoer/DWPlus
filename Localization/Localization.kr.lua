if GetLocale() == "koKR" then
  ...
  core.L = {
    ...
    MINUTES = "의사록",
    MISCSETTINGS = "기타 설정",
    DWPSCALESIZE = "모놀리스 DKP 스케일 크기",
    DWPSCALESIZETTDESC = "Monolith DKP 창의 스케일. \"설정 저장\"을 클릭하여 크기를 설정 값으로 변경하십시오.",
    DWPSCALESIZETTWARN = "UI 스케일 (예 : TukUI, ElvUI 등)을 수정하는 다른 애드온을 사용하는 경우 저장 후 / 재로드가 필요할 수 있습니다.",
    MORE = "더",
    ...
    임원이 필요한 권한을 제한하려면이 화이트리스트 설정 만 사용하는 것이 좋습니다. 모든 임원에게 권한을 부여하려면이 설정 기능을 완전히 무시하십시오. 주의해서 사용하십시오. (이 설정을 적용하면 문제가 발생하면 텍스트 편집기로 \WTF\Accounts\ACCOUNT_NAME\SavedVariables\DWPlus.lua 파일을 열고 하단 근처의 DWPlus_Whitelist 테이블을 삭제하십시오.)]=],
    WIPETABLES = "테이블 정리",
